...
@Form.create()
@connect(({ dslTree }) => ({
  length: dslTree.length,
  total: dslTree.total,
  purcharses: dslTree.purcharses,
  entities: dslTree.entities,
}))
class DSLTREE extends Component {
  state = {
    expandedKeys: [],
      ...
      <PageHeaderWrapper>
        <Card style={{ marginTop: '20px' }}>
          <div style={{ fontSize: '17px', margin: '4px' }}>
            搜检模型：相关度评分 + log(1 + scoreSaleQuantity) + log(1 +
            scoreServiceProtocolQuantity) + log(1 + scoreSaleQuantity)(只浙江省生效) +命中品牌*20 ）
          </div>
...
export default Form.create()(DSLTREE);
